...
<div class="m-auto flex w-[calc(100%-2rem)] max-w-md flex-col gap-4 rounded-xl border p-6 sm:p-7">
	<h1 class="text-2xl font-medium">Enter your new password</h1>
	<form method="post" use:enhance class="flex flex-col gap-2">
		<label for="form-reset.password" class="text-sm font-light">Password</label>
		<input
			type="password"
		...
		>
		{#if form?.message}<p class="text-sm">{form.message}</p>{/if}
	</form>
</div>
